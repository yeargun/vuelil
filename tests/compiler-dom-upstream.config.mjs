...
const aliases = Object.fromEntries(
  Object.entries(vueSourceEntries).filter(
    ([name]) => !["@vue/compiler-core", "@vue/compiler-dom"].includes(name),
  ),
);
...
export default defineConfig({
  root: upstreamRoot,
  plugins: [{
    name: "vuelil-compiler-dom-foundation-candidate",
    enforce: "pre",
    ...
    load(id) {
      const path = clean(id);
      if (isAbsolute(path) && inside(path, compilerDomSource)) {
        throw new Error(`blocked upstream compiler-dom implementation: ${path}`);
      }
      if (isAbsolute(path) && inside(path, compilerCoreSource)) {
        throw new Error(`blocked upstream compiler-core implementation: ${path}`);
      }
      return null;
    },
  }],
