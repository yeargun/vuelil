...
const compilerSSRSource = resolve(upstreamRoot, "packages/compiler-ssr/src");
...
const compilerSSRCandidate = resolve(
  labRoot,
  "tests/compiler-ssr-upstream.candidate.mjs",
);
const compilerDOMCandidate = resolve(labRoot, "packages/vuelil/compiler-dom.js");
...
export default defineConfig({
  root: upstreamRoot,
  plugins: [{
    name: "vuelil-compiler-ssr-candidate",
    enforce: "pre",
    resolveId(source, importer) {
      if (targets(source, importer, "@vue/compiler-ssr", compilerSSRSource)) {
        return compilerSSRCandidate;
      }
    ...
    load(id) {
      const path = clean(id);
      for (const sourceRoot of [compilerSSRSource, compilerDOMSource, compilerCoreSource]) {
